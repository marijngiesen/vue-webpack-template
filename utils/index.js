...
exports.runTslintFix = function runTslintFix(cwd, data, color) {
  if (data.tslint && lintStyles.indexOf(data.tslintConfig) !== -1) {
    console.log(`\n\n${color('Running tslint --fix to comply with chosen preset rules...')}`)
    console.log('# ========================\n')
    return runCommand('npm', ['run', 'tslint', '--', '--fix'], {
      cwd
    })
  }
  return Promise.resolve()
...
/**
 * Returns true if the user will have to run tslint --fix themselves.
 * @param {Object} data Data from questionnaire.
 */
function requiresTslint(data) {
  return !data.autoInstall && data.tslint && lintStyles.indexOf(data.tslintConfig) !== -1
}
